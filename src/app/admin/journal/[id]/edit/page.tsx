...
export default async function EditJournalPage({ params }: Props) {
  ...
  const { data: entry, error } = await supabase
    ...
    .select("id, slug, title, body_markdown, entry_date, status")
    .eq("id", id)
    .maybeSingle();
  ...
  return (
    <section className="admin-content">
      <header className="archive-intro">
        <p className="section-label">Journal · {entry.status}</p>
        <h1>Edit entry</h1>
        <p>Update this entry, save a draft, or change its publication state.</p>
        <Link href={`/admin/journal/${id}/media`}>Manage Journal photos</Link>
      </header>
      <form action={update} className="admin-form">
        ...
        <label>
          Body
          <textarea name="body_markdown" rows={18} defaultValue={entry.body_markdown ?? ""} />
        </label>
        ...
        <div>
          ...
          <button name="intent" value="unpublish" type="submit">Unpublish</button>
          <button name="intent" value="archive" type="submit">Archive</button>
          <Link href="/admin/journal">Back</Link>
        </div>
      </form>
    </section>
  );
