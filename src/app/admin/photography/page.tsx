    ...
    <div className={styles.shell}>
      <main className="admin-page">
        <nav className="admin-page-nav" aria-label="Admin navigation">
          <Link href="/admin">Dashboard</Link><Link href="/admin/photography" aria-current="page">Photography</Link><Link href="/admin/media">Media library</Link><Link href="/admin/media/upload">Upload media</Link>
        </nav>
        <header className="admin-page-header">
          <div><p className="section-label">Admin / Photography</p><h1>Photography collections</h1><p>Build and publish the collections shown on the public Photography archive.</p></div>
          <div className="admin-header-actions"><Link href="/photography" className="admin-button">View public archive</Link><Link href="/admin/media/upload" className="admin-button">Upload photographs</Link></div>
        </header>
        <section className="admin-form-card"><div className="admin-section-heading"><h2>New collection</h2><span>Draft by default</span></div><form action={createCollection} className="admin-form-grid"><label>Title<input name="title" required placeholder="Monsoon light" /></label><label>Slug<input name="slug" placeholder="monsoon-light" /></label><label>Location<input name="location" placeholder="Jaipur" /></label><label>Date<input name="collection_date" type="date" /></label><label>Status<select name="status" defaultValue="draft"><option value="draft">Draft</option><option value="published">Published</option><option value="private">Private</option><option value="archived">Archived</option></select></label><label className="admin-form-wide">Description<textarea name="description" rows={3} placeholder="A short description of this collection." /></label><div className="admin-form-actions"><button type="submit" className="admin-button">Create collection</button></div></form></section>
        <section className="admin-stack">
          {typedCollections.length === 0 ? <div className="admin-empty"><h2>No collections yet.</h2><p>Create your first collection above, then attach images from the media library.</p></div> : typedCollections.map((collection) => { const attached = new Map(collection.photo_collection_items.map((item) => [item.media_id, item])); const isArchived = collection.status === "archived"; return <form key={collection.id} action={saveCollection} className="admin-form-card"><input type="hidden" name="id" value={collection.id} /><input type="hidden" name="archive_status" value={collection.status} /><div className="admin-section-heading"><h2>{collection.title}</h2><span>{collection.status}</span></div><div className="admin-collection-actions"><Link href={`/photography?collection=${collection.slug}`} className="admin-text-link">Preview collection →</Link><button formAction={archiveCollection} className="admin-text-button">{isArchived ? "Restore collection" : "Archive collection"}</button></div><div className="admin-form-grid"><label>Title<input name="title" defaultValue={collection.title} required /></label><label>Slug<input name="slug" defaultValue={collection.slug} required /></label><label>Location<input name="location" defaultValue={collection.location ?? ""} /></label><label>Date<input name="collection_date" type="date" defaultValue={collection.collection_date ?? ""} /></label><label>Status<select name="status" defaultValue={collection.status}><option value="draft">Draft</option><option value="published">Published</option><option value="private">Private</option><option value="archived">Archived</option></select></label><label className="admin-form-wide">Description<textarea name="description" rows={3} defaultValue={collection.description ?? ""} /></label></div><div className="admin-media-picker"><div className="admin-section-heading"><h3>Photographs</h3><span>Select, order, caption</span></div>{typedMedia.map((asset) => { const item = attached.get(asset.id); return <label key={asset.id} className="admin-media-option">{asset.public_url ? <Image src={asset.public_url} alt={asset.alt_text ?? asset.filename} width={72} height={54} className="admin-media-thumbnail" /> : <span className="admin-media-thumbnail-placeholder">No preview</span>}<input type="checkbox" name="media_id" value={asset.id} defaultChecked={Boolean(item)} /><span>{asset.filename}</span><input name={`sort_order_${asset.id}`} type="number" defaultValue={item?.sort_order ?? 0} aria-label={`Order for ${asset.filename}`} /><input name={`caption_${asset.id}`} defaultValue={item?.caption ?? asset.caption ?? ""} placeholder="Caption" aria-label={`Caption for ${asset.filename}`} /></label>; })}</div><div className="admin-form-actions"><button type="submit" className="admin-button">Save collection</button></div></form>; })}
        </section>
      </main>
    </div>
  );
