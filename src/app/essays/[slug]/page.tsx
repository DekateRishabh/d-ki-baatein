...
export async function generateMetadata({ params }: EssayPageProps): Promise<Metadata> {
  ...
  return essay ? { title: essay.title, description: essay.description } : {};
}
    ...
    <>
      ...
      <article className="essay-page">
        <header className="essay-header">
          ...
          <ArticleMeta type={essay.type} readingTime={essay.readingTime} date={essay.date} updated={essay.updated} />
        </header>
        <div className="essay-layout">
          <EssayToc items={[]} />
          <div className="essay-content">
            <figure className="essay-cover-image">
              <Image src={essay.coverUrl ?? "/images/essays/the-things-that-stay.jpg"} alt={essay.coverAlt ?? essay.title} fill priority sizes="(max-width: 760px) 100vw, 720px" />
              <figcaption>{essay.coverCaption ?? ""}</figcaption>
            </figure>
            <div className="essay-body"><MarkdownContent content={essay.bodyMarkdown} /></div>
            <Divider label="End of essay" />
            <section className="related-essays">
              <SectionLabel>Continue reading</SectionLabel>
              <h2>More from the library</h2>
              <p>More essays about memory, attention, places, and the ordinary details that shape a life.</p>
