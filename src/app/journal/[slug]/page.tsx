...
export default async function JournalEntryPage({ params }: JournalPageProps) {
  const { slug } = await params;
  const [entry, entries] = await Promise.all([getPublishedJournalEntryBySlug(slug), getPublishedJournalEntries()]);
  if (!entry) notFound();
  ...
  const relatedEntries = entries.filter((journalEntry) => journalEntry.slug !== entry.slug && journalEntry.category === entry.category).slice(0, 3);
  ...
  return (
    <main className="journal-detail-page">
      <header className="journal-detail-header">
        ...
        <div className="journal-detail-tags"><span>{entry.category}</span>{entry.tags.map((tag) => <Link href={`/journal?tag=${encodeURIComponent(tag)}`} key={tag}>#{tag}</Link>)}</div>
      </header>

      {entry.bodyMarkdown && <article className="journal-detail-body"><MarkdownContent content={entry.bodyMarkdown} /></article>}

      {relatedEntries.length > 0 && <section className="related-journal"><SectionLabel>Related entries</SectionLabel><div className="related-journal-list">{relatedEntries.map((relatedEntry) => <Link href={`/journal/${relatedEntry.slug}`} key={relatedEntry.slug} className="related-journal-item"><span>{relatedEntry.date}</span><strong>{relatedEntry.title}</strong><small>{relatedEntry.category}</small></Link>)}</div></section>}

      <JournalNavigation previous={previousEntry} next={nextEntry} />
      <footer className="journal-detail-footer"><div>{entry.location && <span>{entry.location}</span>}{entry.mood && <span>{entry.mood}</span>}</div><EditorialLink href="/journal">Back to journal</EditorialLink></footer>
    </main>
  );
}
